...
type TuringTransition = tuple[str, Direction, str]
type TuringTransitionTable = dict[str, dict[str, tuple[str, Direction, str]]]
type DFATransitionTable = dict[str, dict[str, str]]
type NFATransitionTable = dict[str, dict[str, set[str]]]
...
class NFA:
    alphabet: set[str]
    transitions: NFATransitionTable
    accepting_states: set[str]
    start: str
    states: set[str] = field(init=False)

    def _get_lambda_closure(
        self, state: str, visited: set[str] | None = None
    ) -> set[str]:
        states = {state}
        ...
        for next_state in self.transitions[state][""]:
            if next_state in visited:
                continue
            states |= self._get_lambda_closure(next_state, visited)
            visited |= {next_state}

        return states

    def powerset_construct(self) -> dict[frozenset[str], dict[str, str]]:
        curr_state = self.start
        i = 0
        while True:
            closure = self._get_lambda_closure(curr_state)
            states = set()
            for state in closure:
                for letter in self.transitions[state]:
                    states |= self.transitions[state][letter]
            i += 1
            if i > 20:
                break
            print(states)
        raise Exception()  # This still hasn't been fixed/implemented fully.
...
class TuringMachine:
    ...
    def from_dfa(cls, dfa: DFA) -> Self:
        ...
        transitions = {
            state: {"⊔": ("⊔", R, reject)}
            | {symbol: (symbol, R, state) for symbol in dfa.transitions[state]}
